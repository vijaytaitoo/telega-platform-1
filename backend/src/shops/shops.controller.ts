import { Controller, Get, Post, Put, Delete, Body, Param, UseGuards, ValidationPipe, UsePipes, ParseUUIDPipe } from '@nestjs/common';
...
export class ShopsController {
  ...
  @ApiResponse({ status: 200, description: 'Магазин успешно найден', type: Shop })
  ...
  @ApiResponse({ status: 201, description: 'Магазин успешно создан', type: Shop })
  @ApiResponse({ status: 400, description: 'Некорректные данные магазина' })
  async create(@Body(ValidationPipe) createShopDto: CreateShopDto): Promise<Shop> {
    return this.shopsService.create(createShopDto);
  }

  @Put(':id')
  @ApiOperation({ summary: 'Обновить магазин' })
  @ApiResponse({ status: 200, description: 'Магазин успешно обновлен', type: Shop })
  @ApiResponse({ status: 400, description: 'Некорректные данные магазина' })
  @ApiResponse({ status: 404, description: 'Магазин не найден' })
  async update(
