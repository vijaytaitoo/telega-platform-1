...
const GlowButton = styled.button<{ theme: 'light' | 'dark' }>`
  ...
  // Анимация пульсации
  &:active {
    animation: glow ${BrandConfig.animation.timing.slow} ${BrandConfig.animation.easing.bounce}
      infinite;
  }
...
const ThemeToggle = styled.button<{ theme: 'light' | 'dark' }>`
  ${BrandConfig.components.common.button.base}
  background: ${(props) =>
    props.theme === 'light' ? BrandConfig.colors.brand.violet : BrandConfig.colors.brand.pink};
...
const NavButton = styled.button<{ theme: 'light' | 'dark' }>`
  ...
  &:hover {
    background: ${(props) =>
      props.theme === 'light' ? 'rgba(255, 79, 150, 0.1)' : 'rgba(255, 79, 150, 0.2)'};
  }
`;
```
